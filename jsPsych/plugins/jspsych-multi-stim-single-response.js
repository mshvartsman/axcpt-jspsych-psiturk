/**
* jspsych-multi-stim-single-response
* Michael Shvartsman
*
* plugin for displaying a set of stimuli and collecting a response on the final one
* via the keyboard
*
*
**/
...
(function($) {
	jsPsych["multi-stim-single-response"] = (function() {
		...
		plugin.create = function(params) {
			...
			var rt_offset = 0; 
			for (var i=0; i<params.timing_stim.length-1; i++){
				rt_offset = rt_offset + params.timing_stim[i]; 
			}
		...
		plugin.trial = function(display_element, trial) {
			...
			function showNextStimulus(){
				...
				if (!trial.is_html) {
					display_element.append($('<img>', {
						...
						id: 'jspsych-multi-stim-single-response-stimulus'
					}));
				} else {
				...
				if (typeof trial.timing_stim[whichStimulus] !== 'undefined' && trial.timing_stim[whichStimulus] > 0) {
					var t1 = setTimeout(function() {
						...
						if(typeof trial.stimuli[whichStimulus + 1] !== 'undefined'){
							...
							$('#jspsych-multi-stim-single-response-stimulus').css('visibility','hidden');
						}
			...
			// start the response listener on the final stimulus
			var keyboardListener; 
			
			tResponse = setTimeout(function() {keyboardListener = jsPsych.pluginAPI.getKeyboardResponse(after_response, allchoices, "date", true); }, trial.rt_offset); 
